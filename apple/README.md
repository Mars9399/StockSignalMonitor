...
- macOS 概览与详情显示“即时买入概率* / 建议减持概率*”，综合价格通道、趋势、动量、RSI、量能、持仓和账户暴露；该百分比是规则评分，不是历史胜率
- macOS“界面”设置支持初始窗口宽高、文字缩放、列表行高、日志高度、日志显隐和启动后自动监控，偏好由 `UserDefaults` 持久化
- macOS 概览、自选、侧栏与全市场列表的价格文字按最近一次变化方向持续显示绿色/红色，直到下一次反向变化；不使用定时闪烁
- macOS“全市场股票”入口可切换 Yahoo 美股/港股市场并分页浏览，支持代码/名称搜索，右键或双击股票可加入自选
- 至少 20 根完整日线后启用价格通道；单只股票请求失败不会中断全部行情，连接异常自动重试
